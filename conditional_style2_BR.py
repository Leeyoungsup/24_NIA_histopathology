...
train_dataset=CustomDataset(params,train_images,F.one_hot(torch.tensor(image_label)))
dataloader=DataLoader(train_dataset,batch_size=params['batch_size'],shuffle=True)


generator = stylegan.Generator(
    z_dim=1024,  # Input latent (Z) dimensionality
    c_dim=len(class_list),    # Conditioning label (C) dimensionality (0 = no labels)
...
checkpoint=torch.load(f'../../model/styleGan2/BRNT/checkpoint_epoch_50.pt',map_location=device)
generator.load_state_dict(checkpoint['generator_state_dict'])
discriminator.load_state_dict(checkpoint['discriminator_state_dict'])
g_optimizer.load_state_dict(checkpoint['g_optimizer_state_dict'])
d_optimizer.load_state_dict(checkpoint['d_optimizer_state_dict'])
checkpoint=0
for epc in range(51,params['epochs']):
    gloss_total = 0
    dloss_total = 0
    step = 0
 ...
 # 이미지 생성 및 저장
    ...
    with torch.no_grad():
        for cls_idx, cls_name in enumerate(class_list):
            ...
            save_image(transback(generated_images), f'../../result/styleGan2/BRNT/{cls_name}/generated_images_epoch_{epc+1}.png', nrow=4)
    ...
    torch.save(checkpoint, f'../../model/styleGan2/BRNT/checkpoint_epoch_{epc+1}.pt')
