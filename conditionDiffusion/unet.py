...
class AttnBlock(nn.Module):
    def __init__(self, in_ch: int):
        ...
        self.proj_q = nn.Conv2d(in_ch, in_ch, kernel_size=1, stride=16, padding=0)
        self.proj_k = nn.Conv2d(in_ch, in_ch, kernel_size=1, stride=16, padding=0)
        self.proj_v = nn.Conv2d(in_ch, in_ch, kernel_size=1, stride=16, padding=0)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h = self.proj(h)
        h = F.interpolate(h, size=(H, W), mode='bilinear', align_corners=False)  # 원래 크기로 업샘플링
