...
checkpoint = torch.load(
    f'../../model/conditionDiff/color_scratch_details/STNT/ckpt_133_checkpoint.pt', map_location=device)
...
count = {key: 100000 for key in class_list}
while (True):
    ...
    with torch.no_grad():
        lab = torch.ones(len(class_list), each_device_batch // len(class_list)).type(torch.long) \
            * torch.arange(start=1, end=len(class_list)).reshape(-1, 1)
