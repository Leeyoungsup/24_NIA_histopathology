...
checkpoint = torch.load(
    f'../../model/conditionDiff/color_scratch_details/BRIL/ckpt_207_checkpoint.pt', map_location=device)
...
count = {key: 102160 for key in class_list}
while (True):
    ...
    with torch.no_grad():
        ...
        # lab = torch.tensor([[1, 2, 1, 2], [1, 2,1, 2]], dtype=torch.long)
        ...
        for i in range(len(lab)):
            ...
            img_pil.save(
                params['data_path']+f'{class_list[lab[i]]}/NIA_S_BRIL_{class_list[lab[i]][2:]}_{str(count[class_list[lab[i]]]).zfill(6)}.jpeg')
