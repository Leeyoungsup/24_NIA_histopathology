...
for epc in range(start_epc, params['epochs']):
    ...
    with torch.no_grad():
        ...
        generated =transback(Generator(transback(SR_model(transback(generated)))*2-1))
        for i in range(len(lab)):
            img_pil = topilimage(generated[i].cpu())
            createDirectory(
                f'../../result/custom_detail1/BRIL/{class_list[lab[i]]}')
            img_pil.save(
                f'../../result/custom_detail1/BRIL/{class_list[lab[i]]}/{epc}.png')
    ...
    createDirectory(
        f'../../model/conditionDiff/custom_detail1/BRIL/')
    torch.save(
        checkpoint, f'../../model/conditionDiff/custom_detail1/BRIL/ckpt_{epc+1}_checkpoint.pt')
